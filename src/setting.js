let globalSettings = {
    ...
    copyrightDates: '2022 shellwe',
    copyrightCompany: '广州大学华软软件学院软件工程系毕业设计(不得商用)',
    copyrightWebsite: 'https://shellwe.top',
    // 是否开启导航搜索
    ...
    // 控制台名称
    dashboardTitle: '用户后台管理系统',
    // 是否开启主题配置（建议在生产环境关闭）
    enableThemeSetting: true
}

// 演示&开发环境开启全部功能（这部分代码可删除，仅方便打包演示环境）
if (process.env.VUE_APP_TYPE == 'example' || process.env.NODE_ENV == 'development') {
    ...
    if (process.env.VUE_APP_TYPE == 'example') {
        globalSettings.copyrightWebsite = '/HR-admin'
    }
}
